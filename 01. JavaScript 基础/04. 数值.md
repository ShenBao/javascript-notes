...
如果指数部分等于或超过最大正值1024，JavaScript 会返回`Infinity`，这称为“正向溢出”；如果等于或超过最小负值-1023（即非常接近0），JavaScript 会直接把这个数转为0，这称为“负向溢出”。
...
至于具体的最大值和最小值，JavaScript 提供`Number`对象的`MAX_VALUE`和`MIN_VALUE`属性表示。
